...
import { PROMPT, SelectNoOfPersons } from "../../constants/Options";
import { Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle, DialogClose, DialogFooter } from "@/components/ui/dialog";
import { FcGoogle } from "react-icons/fc";
import { AiOutlineLoading3Quarters } from "react-icons/ai";
import { Button } from "@/components/ui/button";
import toast from "react-hot-toast";
import { AIService } from "@/Service/AiModel";
...
class TripCreator {
  ...
  static validateForm(formData) {
    ...
    if (formData?.noOfDays > 7) {
      throw new Error("Максимальна кількість днів - 7");
    }
...
function CreateTrip({createTripPageRef}) {
  ...
  const generateTrip = async () => {
    ...
    try {
      ...
      const tripData = await ai.generateFromPrompt(TripCreator.generatePrompt(formData));

      const tripId = await tripCreator.saveTrip(formData, tripData);
